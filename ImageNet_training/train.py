...
def evaluate(model, criterion, data_loader, device, print_freq=100):
    ...
    log_string(' * Acc@1 {top1.global_avg:.3f} Acc@5 {top5.global_avg:.3f}'
...
def load_data(traindir, valdir, cache_dataset, distributed):
    ...
    if cache_dataset and os.path.exists(cache_path):
        ...
        log_string("Loading dataset_train from {}".format(cache_path))
        ...
        if cache_dataset:
            log_string("Saving dataset_train to {}".format(cache_path))
            utils.mkdir(os.path.dirname(cache_path))
            utils.save_on_master((dataset, traindir), cache_path)
    log_string("Took "+str(time.time() - st))

    log_string("Loading validation data")
    cache_path = _get_cache_path(valdir)
    if cache_dataset and os.path.exists(cache_path):
        # Attention, as the transforms are also cached!
        log_string("Loading dataset_test from {}".format(cache_path))
        ...
        if cache_dataset:
            log_string("Saving dataset_test to {}".format(cache_path))
            utils.mkdir(os.path.dirname(cache_path))
            utils.save_on_master((dataset_test, valdir), cache_path)

    log_string("Creating data loaders")
    if distributed:
...
def parse_args():
    ...
    parser.add_argument('--data-path', default='/media/ssd/datasets/ILSVRC/Data/CLS-LOC', help='dataset')
